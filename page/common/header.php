<header>
    <a class="header__left" href="<?php echo SITE_URL; ?>/page/index">
        <img width="28" height="28" class="container__brand" src='<?php echo SITE_URL; ?>/img/logo.png' alt="Logo">
        <h1>Lambda</h1>
    </a>
    <nav class="header__right">
        <?php if (isset($_SESSION['name']) && isset($_SESSION['password'])){ ?>
                        ...
                        xhr.onreadystatechange = () => {
                            if (xhr.readyState == 4 && xhr.status == 200) {
                                ...
                                for (var i = 0; i < response['name'].length; i++) {
                                    document.getElementById("search_user__list").innerHTML+="<a class='dropdown__panel__item' href='<?php echo SITE_URL; ?>/page/index/index.php?content_type=user&id="+response['id'][i]+"'>"+response['name'][i]+"</a>";
                                }
                            }
                        }
            ...
            <div class="dropdown hover header__right__item">
                <img class="dropdown__item" width="28" height="28" src="<?php echo SITE_URL; ?>/img/notif.png" alt="notif">
                <div class="dropdown__panel overflow">
                    <?php
                        ...
                        while($line = $response->fetchArray()){
                            if($line['type'] == 'follow'){ ?>
                                <a class="dropdown__panel__item" onclick="sup_notif(<?php echo $line['ID']; ?>)" href="<?php echo SITE_URL; ?>/page/index?content_type=user&id=<?php echo $line['user_concerning']; ?>">
                                    <?php echo get_username($line['user_concerning']); ?> a commencé à vous suivre
                                </a>
                    <?php   }
                            if($line['type'] == 'follow request'){ ?>
                                <a class="dropdown__panel__item" href="#">
            ...
            <div class="dropdown hover header__right__item">
                ...
                <div class="dropdown__panel">
                    <a class="dropdown__panel__item" href="<?php echo SITE_URL; ?>/page/index?content_type=user&id=<?php echo $_SESSION['user_ID']; ?>">Mon profil</a>
                    <a class="dropdown__panel__item" href="<?php echo SITE_URL; ?>/page/index?content_type=settings">Options</a>
                    <hr>
                    <a class="dropdown__panel__item red" href="<?php echo SITE_URL; ?>/page/sign_in.php">Se déconnecter</a>
                </div>
            </div>
        <?php }else{?>
            <a href="<?php echo SITE_URL; ?>/page/sign_in.php" class="button is-primary">Se connecter</a>
        <?php }?>
    </nav>
</header>
...
<script type="text/javascript">
    function sup_notif(ID_sup, user_send) {
        ...
        xhr.onreadystatechange = () => {
            if (xhr.readyState == 4 && xhr.status == 200) {
                window.location.assign("<?php echo SITE_URL; ?>/page/index?content_type=user&id="+user_send);
            }
        }
        xhr.open("POST", '<?php echo SITE_URL; ?>/cible/delete_notif_view.php', true);
        xhr.send(data);
    }
